...
UniGather 是一款面向公司总部工作人员的 Windows 单机桌面应用，包含“收集任务”和“邮件发送”两个独立工作台：前者集中收取反馈材料并追踪单位状态，后者扫描本地材料目录并按通讯录逐单位发送。

> 当前版本为 Windows 单机版，可配置邮箱并通过 IMAP/POP3 收件、归档附件和追踪单位反馈。

新手操作手册：

- [Markdown 版使用手册](docs/UniGather-新手使用操作手册.md)
- [Word 版使用手册](docs/UniGather-新手使用操作手册.docx)

- GitHub 项目：<https://github.com/cwxsss/unigather>
- v0.0.8 发布页：<https://github.com/cwxsss/unigather/releases/tag/v0.0.8>
- v0.0.7 发布页：<https://github.com/cwxsss/unigather/releases/tag/v0.0.7>
...
- **立即刷新与定时收件**：任务详情可立即按起始时间至截止时间补收并匹配邮件；应用运行或托盘驻留期间，按任务设置的轮询间隔自动增量收件，截止时间后不再收取。
...
2. 打开“通讯录”，下载模板并填写单位、联系人、邮箱、电话和单位别名，导入 Excel 或 CSV 文件；旧版单位/邮箱、单位/姓名/邮箱模板仍兼容。
3. 打开“收集任务”，点击“新建任务”，填写任务名称、主题关键词和截止时间，并在“选择单位”中全选或单选本次涉及的单位。
4. 保存后可点击任务行查看详情和进展；点击“立即刷新”可以立刻补收并匹配该任务时间范围内的邮件。
5. 任务的“每 30 分钟”表示自动轮询间隔，应用运行或托盘驻留时会按此间隔收件；之后仍可编辑任务并调整单位范围。
6. 在任务中心查看任务状态；删除任务使用每行右侧的 `×` 按钮。
7. 同步完成后，在“材料归档”中导出材料清单。
...
“邮件发送”模块与收集任务完全解耦，支持以下流程：

1. 在“邮箱配置”的“发信连接”中设置 SMTP 服务器、端口和 SSL/TLS 或 STARTTLS；代理与账号密码和收件共同复用。
2. 在“邮件发送”选择材料目录并扫描（可递归子目录），系统按单位名称/别名/本地模糊规则匹配文件。
3. 对待确认或未匹配文件人工改派；同一单位的多个附件合并到一封邮件，多个邮箱合并到 To，批次抄送统一放入 CC。
4. 填写主题、正文和测试邮箱，先启动检查、发送测试；确认无误后输入“确认发送”执行正式发送。
5. 历史批次保留发送结果，失败项可单独重试；附件内容不会上传到 AI，低置信度项目必须人工确认。

建议先填写测试收件邮箱并点击“发送测试邮件”。测试成功后，正式发送仍需输入“确认发送”。发件账号密码不会写入 SQLite，而是从 Windows 凭据管理器读取；若提示未保存密码，请先在“邮箱配置”中保存同一账号的密码。

主题和正文支持 `{单位名称}` 占位符。发送进度会按单位实时显示，发送结果会写入本地 SQLite 的 `send_runs` 和 `send_items` 表，便于后续追溯。
